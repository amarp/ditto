...
void read_traffic_requirement_file(char *filename)
{
  char buf[1024];
  char tmp[1024];
  struct token_struct{
  ...
  while (fgets(buf, 1024, fp) != NULL) 
	...
	temp = strtok(NULL,"\t ");
      ...
      if(strcmp(token[0].buffer,"TRAFFICENTRY") == 0){
	/*
	  Format of the traffic file
	  TRAFFICENTRY     SRC      DST      WT.TRAFFIC 
	*/
	
	add_link(atoi(token[1].buffer), atoi(token[2].buffer), atof(token[3].buffer), traffic_graph,0,0,0);
      }
    }
  
  printf("read traffic file...Traffic graph is -->\n");
  dump_graph(traffic_graph);
}


void gen_traffic_per_link()
{
  /* 
     The basic algo is to run through all the traffic node pair requirements
     Annonate each link from source to destination with the traffic 
  */
  ...
  for(i = 1; i <= NUM_NODES; i++) {
    for(l = traffic_graph->lh[i].lh_first; l; l = l->ln_link.le_next) {
      source = i;
      hop = 1;
      destination = l->ln_dst;
      cost = l->ln_cost;
      //printf("->%d %f",l->ln_dst, l->ln_cost);
      
      reset_route(temproute);
 
      route = get_route(source, destination, metric_graph, temproute, NUM_NODES+1);
      printf("\nPath from %d to %d is :\n %d -> ", source, destination, source);
      do {
	next = route[hop++];
	traffic_cost[source][next] += cost;
	source = next;
	printf("%d  %f-> ", next, cost);
      }while(next != destination);


      /*
	Run through the path and anotate each link with cost from source to destination.
       */
...
void print_link_traffic()
{
  FILE *lt = fopen("LINK_TRAFFIC","w");
  int i = 0, j = 0;

  for(i = 1 ; i <= NUM_NODES; i++){
    for(j = 1 ; j <= NUM_NODES; j++){

      fprintf(lt, "%d\t%d\t%f\n", i, j, traffic_cost[i][j]);
      printf("%d\t%d\t%f\n", i, j, traffic_cost[i][j]);
    }
...
void usage()
{
  printf("./swarm <conflict information file> <metric information file> <NUM NODES> <NUM_GATEWAYS> <traffic file>\n");
  exit(1);
}
...
int main(int argc,char *argv[])
{
  ...
  NUM_GATEWAYS = atoi(argv[4]);
  traffic_cost = (double **)malloc(sizeof(double *)*(NUM_NODES+2));
  for (i = 0 ; i <= NUM_NODES + 1; i++){
    traffic_cost[i] = (double *)malloc(sizeof(double)*(NUM_NODES+2));
    //memset(traffic_cost[i], 0x0, (NUM_NODES+2)*sizeof(double));
  }
  ...
  for(i = 1; i <= NUM_NODES; i++) {
    ...
    if (i == 6 || i == 8 || i == 9 || i == 10 || i == source || i == 12 ||  i == 17 || 
	i == 20 || i == 21 || i == 23 || i == 24 || i == 26 || 
	i ==31 || i == 32 || i == 33 || i == 35 || i == 36) {
      continue;
    }
    reset_route(temproute);
