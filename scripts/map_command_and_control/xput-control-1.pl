...
#scenario2
my @recvrs = ("map1", "map3", "map4", "map5", "map7", "map14", "map15", "map16", "map27", "map28");
...
for (my $i = 0; $i < $NUM_AVGS; $i++) {
	...
	for (my $j = 0; $j <= $#recvrs; $j++) {
	    ...
	    my @bad = ();
	    check_recvrs(\@temp_recv, \@oids, \@bad);
	    print "--------------------------------------------\n";
	...
	system("date");
	system("scp -r expt* aphanish\@moo.cmcl.cs.cmu.edu:/disk/agami1/aphanish/share/ditto_logs/map/");
