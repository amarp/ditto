...
if ARGV.size < 6
    puts "Usage: ./analyze_1.rb <testbed> <test_dir_relative_path> <recvr_list> <num_runs> <zip? (1/0)> <sniffer list>"
    puts "Usage e.g.: ./analyze_1.rb emulab \"./emulab/300KB/\" recvrs.txt 3 1 sniffers.txt"
    ...
    puts "    Currently records the following stats :"
...
recvrs.each { |recvr|
    ...
    for chunk_size in chunk_sizes
        ...
        for comb in schemes
            ...
            pretty_str = a_total_xput_display.join(' ')
                ...
                if other_recvr != recvr
                    ...
                    for run_number in 0..(numRuns-1)
                        ...
                        if line_count.to_i > 0
                            ...
                            # if any othe files expected here are not present, 
                            # .to_i or .to_f will make sure that the empty stiring returned from the cmd is turned into 0 !
                            ...
                            if numBytesOfTcpPacketsCaptured.to_i > 0
                                ...
                                #useless_percent = ((numBytesOfTcpPacketsCaptured.to_f - numBytesOfChunksSniffed.to_f) / numBytesOfTcpPacketsCaptured.to_f) * 100
                            end
                    ...
                    pretty_str = "#{other_recvr} " + a_percentage_chunks_sniffed_display.join(' ')
                    append_agg(a_percentage_chunks_sniffed, out_c_percent, pretty_str)

                    pretty_str = "#{other_recvr} " + a_num_chunk_stats_display.join(' ')
                    append_agg(a_num_chunk_stats, out_c, pretty_str)

                    pretty_str = "#{other_recvr} " + a_num_chunk_bytes_stats_display.join(' ')
                    append_agg(a_num_chunk_bytes_stats, out_cb, pretty_str)

                    pretty_str = "#{other_recvr} " + a_num_tcp_bytes_stats_display.join(' ')
                    append_agg(a_num_tcp_bytes_stats, out_tb, pretty_str)

                    pretty_str = "#{other_recvr} " + a_useless_percent_stats_display.join(' ')
                    #puts pretty_str
