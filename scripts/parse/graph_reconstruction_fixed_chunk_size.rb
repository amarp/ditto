...
if ARGV.size < 1
    puts "Usage: ./graph_reconstruction_fixed_chunk_size.rb <results_dir - the directory which has \"results\">"
...
for chunk_size in chunk_sizes
    ...
    gtype_tags.each_index { |i|
        ...
        schemes.each_index { |j|
            ...
            puts "'#{GRAPH_DATA_DIR}/#{chunk_size}_#{comb}.cdf' using 1:2 with lines #{line_styles[j]} title '#{comb_tag}'"
            plots << "'#{GRAPH_DATA_DIR}/#{chunk_size}_#{comb}.cdf' using 1:2 with lines #{line_styles[j]} title '#{comb_tag}'"
        }
        plots = plots.join(', ')
