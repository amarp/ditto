...
schemes = ['proxy-mhear', 'proxy-ohear']
schemes_tags = ['Ditto', 'Ditto (w/o inter-stream reassembly)']
...
if ARGV.size < 1
    puts "Usage: ./graph_useless_percent_fixed_chunk_size.rb <results_dir - the directory which has \"results\">"
    puts "Precondition - needs the files that have cdf data for various schemes (do, po, pm). Make sure to run generate_wasted_bytes_cdf.rb"
    exit
end
...
for chunk_size in chunk_sizes
    ...
    gtype_tags.each_index { |i|
        ...
        schemes.each_index { |j|
            ...
            puts "'#{GRAPH_DATA_DIR}/useless_percent_#{chunk_size}_#{comb}.cdf' using 1:2 with lines #{line_styles[j]} title '#{comb_tag}'"
            plots << "'#{GRAPH_DATA_DIR}/useless_percent_#{chunk_size}_#{comb}.cdf' using 1:2 with lines #{line_styles[j]} title '#{comb_tag}'"
        }
        plots = plots.join(', ')
...
set size 0.65
set output '#{GRAPH_DIR}/useless_percent_#{chunk_size}_#{gtype}.eps'
