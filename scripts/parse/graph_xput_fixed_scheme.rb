...
if ARGV.size < 1
    puts "Usage: ./graph_xput_fixed_scheme.rb <results_dir - the directory which has \"results\">"
    puts "Precondition - needs the files that have cdf data for various schemes Run generate_xput_cdf_data.rb"
    exit
end
...
for comb in schemes
    ...
    gtype_tags.each_index { |i|
        ...
        chunk_sizes.each_index { |j|
            ...
            puts "'#{GRAPH_DATA_DIR}/xput_#{chunk_size}_#{comb}.cdf' using 1:2 with lines #{line_styles[j]} title '#{chunk_size}B'"
            plots << "'#{GRAPH_DATA_DIR}/xput_#{chunk_size}_#{comb}.cdf' using 1:2 with lines #{line_styles[j]} title '#{chunk_size}B'"
        }
        plots = plots.join(', ')
...
set size 0.65
set output '#{GRAPH_DIR}/xput_#{comb}_#{gtype}.eps'
#set title "#{comb}"
