...
my @combs = ("proxy-mhear", "proxy", "dot");
my $chunks = "8K";

my @files = ("dot_snap_20070206.tar.gz", "coreutils-6.9.tar.bz2"); #, "FILES/gcc2", "FILES/gcc3");
my @oids = ("5c6bac26d0f356b3590b786972a742561644b202", "10310a78c4e1d9f742f3815ac28e9bf1086a5885"); #, "efgh", "ijkl");
...
#my @filenums = ("0", "0", "0", "0");
#my @times = ("0", "30", "200", "200");

my @recvrs = ("nodew1", "nodew2", "nodew3", "nodew5", "nodew7");
#my @filenums = ("1", "1", "1", "1", "1");
my @filenums = ("0", "0", "0", "0", "0");
#my @times = ("0", "200", "200", "200", "200");
my @times = ("0", "0", "0", "0", "0");
...
if( $#ARGV < 1 ) { die "perl control-1.pl <list of nodes> <expt-dir>"; }
...
for (my $i = 0; $i < $NUM_AVGS; $i++) {
    @recvrs = shuffle(@recvrs);
    print "@recvrs";
    foreach my $c (@combs) {

	print "PERRUN chunk = $chunks, comb = $c, run = $i\n";
	system("date");
	...
	run_dot($node_list, \@allhosts, $c, $chunks); # amar:: start sniffTcp on all nodes?
	print "--------------------------------------------\n";
	    
	print "-----> Starting sender GCP\n";
	run_sender($GATEWAY, \@files);
	print "--------------------------------------------\n";
	
	print "-----> Starting receiver GCP\n";
	for (my $j = 0; $j <= $#recvrs; $j++) {
	    ...
	    my $r = $recvrs[$j];
	    my $oid = $oids[$filenums[$j]];
	    
	    run_recvr($r, $oid, $GATEWAY);
	}
	print "--------------------------------------------\n";
	...
	get_stuff($node_list, "$expt_dir-$chunks-$c-$i");
	print "--------------------------------------------\n";

	print "-----> Figuring out the problems\n";
	figure_out(\@recvrs, \@oids, \@bad, $GATEWAY, "$expt_dir-$chunks-$c-$i");
	print "--------------------------------------------\n";
