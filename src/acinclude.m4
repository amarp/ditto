...
AC_MSG_CHECKING([for SFS libraries])
if test -f ${with_sfs}/Makefile -a -f ${with_sfs}/autoconf.h; then
	...
	*) sfssrcdir="${with_sfs}/${sfssrcdir}" ;;
    ...
    AC_MSG_RESULT(${with_sfs})
elif test -f ${with_sfs}/include/sfslite/autoconf.h \
	-a -f ${with_sfs}/lib/sfslite/libasync.la; then
    sfsincludedir="${with_sfs}/include/sfslite"
    sfslibdir=${with_sfs}/lib/sfslite
    if egrep '#define DMALLOC' ${sfsincludedir}/autoconf.h > /dev/null; then
	test -z "$with_dmalloc" -o "$with_dmalloc" = no && with_dmalloc=yes
    else
	with_dmalloc=no
